...
describe("POST /event/join (joinEvent controller)", () => {
  ...
  it("debería permitir a un usuario unirse a un evento con datos válidos", async () => {
    const response = await request(app)
      ...
      .send({
        eventId: "1cc549b1-15b7-460d-a02c-154dc024155c", // <-- id de evento existente
        amount: 1,
      });
    ...
    expect(response.body.user_event).toHaveProperty("eventId", "0b059689-a148-4b02-b087-8cffa23799a2"); // <-- id de usuario del token
    expect(response.body.user_event).toHaveProperty("userId", "e3941e30-4d17-46d1-841d-fbafd35623d3"); // <-- id del evento utilizado
  }, 10000);
